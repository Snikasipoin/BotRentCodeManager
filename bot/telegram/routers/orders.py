...
from bot.telegram.keyboards.main import ORDERS, order_actions
...
async def _active_orders_text(session: AsyncSession) -> tuple[str, list[Order]]:
    orders = (await session.scalars(select(Order).where(Order.status.in_([OrderStatus.PENDING_PHOTO, OrderStatus.ACTIVE, OrderStatus.CREDS_SENT])).order_by(Order.id.desc()))).all()
    if not orders:
...
async def order_approve(callback: CallbackQuery, order_processor: OrderProcessor) -> None:
    ...
    await callback.message.answer(f"Заказ #{order_id} подтвержден и данные отправлены покупателю.")
